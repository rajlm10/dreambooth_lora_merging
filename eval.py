...
evaluator = ImageDirEvaluator("cuda:0")
...
# Required positional argument
parser.add_argument('src_images', type=str,
                    help='Path to original samples in dataset')

# Required positional argument
parser.add_argument('target_text', type=str,
                    help='Text prompt used to train Dreambooth')

args = parser.parse_args()

print("Argument values:")
print(args.gen_samples)
print(args.src_images)
print(args.target_text)
print()

images = []

for img in os.listdir(args.gen_samples):
  image = Image.open(args.gen_samples+'/'+img) 
  transform = transforms.Compose([ 
    transforms.PILToTensor(),transforms.Resize((1024,1024))]) 
  img_tensor = transform(image).to(torch.float32)
  images.append(img_tensor)

gen_samples = torch.stack([img for img in images])


images = []

for img in os.listdir(args.src_images):
  image = Image.open(args.src_images+'/'+img) 
  transform = transforms.Compose([ 
    transforms.PILToTensor(),transforms.Resize((1024,1024))]) 
  img_tensor = transform(image).to(torch.float32)
  images.append(img_tensor)

src_images = torch.stack([img for img in images])

sim_samples_to_img, sim_samples_to_text = evaluator.evaluate(src_images,gen_samples,args.target_text)

print(f'CLIP-I :{sim_samples_to_img}, CLIP-T:{sim_samples_to_text}')
